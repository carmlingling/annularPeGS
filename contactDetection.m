...
f.roach = false;
%% file housekeeping
if not(isfolder(append(p.topDir,p.contactDir))) %make a new folder with particle centers
        mkdir(append(p.topDir,p.contactDir));
end



%disp([p.topDir, 'warpedimg/', p.imgReg]) %print the filename
files = dir([ p.topDir,p.warpedImgDir, '*.tif']); %images
...
for imgnumb = 1:size(files,1)
    ...
    %read in image
    Img = imread([p.topDir,p.warpedImgDir,files(imgnumb).name]);
    Rimg = Img(:,:,1);
    ...
    if f.boundaryType == "annulus"
        ...
        if f.calibrate == true
           ...
           axis on
           hold on
           plot([f.polarizerstrip(1,:) f.polarizerstrip(1:1)],[f.polarizerstrip(2,:) f.polarizerstrip(2,1)],'b','LineWidth',1)
           drawnow;
        end
...
%% initialize data structure
    ...
    if ~isempty(pData)
        ...
        for l = 1:length(f1)
            ...
            %each particle
            [~, locs] = peakfinder(x(1), y(1), r(1), f1(l), Gimgfine, f.minpeakheight, f.minpeakprominence, f.minpeakprom_main, f.padding, f.roach);
            [~, locs2] = peakfinder(x(2), y(2), r(2), f2(l), Gimgfine, f.minpeakheight, f.minpeakprominence, f.minpeakprom_main, f.padding, f.roach);
...
if f.figverbose 
    ...
    for n = 1:length(particle)
        ...
        text(hAx1, particle(n).x, particle(n).y, num2str(particle(n).id), 'Color', 'b')
    end
    drawnow;
...
save([p.topDir,p.contactDir, files(imgnumb).name(1:end-4),'_contacts.mat'],'particle')
...
writecell(params,[p.topDir,p.contactDir,'contactDetect_params.txt'],'Delimiter',',')
