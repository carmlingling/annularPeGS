...
%threshold
function newtonize(p, f, verbose)
...
filename = '*solved.mat';
imname = p.imgReg;
imagefiles = dir([p.topDir,p.synthImgDir, imname]);
%directory = [directory,'warpedimg/'];
forcefiles = dir([p.topDir,p.solvedDir, filename]);
...
% end
if f.boundaryType == "annulus"
    save([p.topDir, p.solvedDir,forcefiles(frame).name(1:end-4),'_update.mat'],'particle');
end
NN = length(particle);
...
imshow(bigSynthImg);
drawnow;
imwrite(bigSynthImg,[p.topDir,p.synthImgDir,imagefiles(frame).name(1:end-4),'update.jpg'])
%%
